    ...
    while True:
        ...
        if np.all(markerIds is not None):  # If there are markers found by detector
            ...
            if h_pos > 640:
                # marker is too far
                if dist_c == 1:
                    wl_c = 1 # left wheel fwd
                    wr_c = 0 # right wheel stops
                # marker is too close
                elif dist_c == -1: # marker is too close
                    wl_c = 0 # left wheel stops
                    wr_c = 1 # right wheel back
            # marker is too left
            elif h_pos < 320: # marker is too left
                # marker is too far    
                if dist_c == 1:
                    wl_c = 0 # left wheel stops
                    wr_c = 1 # right wheel fwd
                # marker is too close
                elif dist_c == -1:
                    wl_c = 1 # left wheel back
                    wr_c = 0 # right wheel stops
            # marker is centered
            elif h_pos >= 320 and h_pos <= 640:
                    # both wheels follow distance coefficient
                    wl_c = 1 
                    wr_c = 1
